...
AABaseModule <- function(input, output, session, credentials, ...) {
	######### Keep these ######################
	ns <- session$ns
	...
	output$mainUI=renderUI({
		list(
			box(width=12, title = "Unloading", align = "center", status = "primary", solidHeader=T
				,inputPanel(
	...
	observeEvent(input$cbSubmit, {
		...
		if (length(cbDockScheduleDT_rows_selected)>0)
		{
			...
			if (length(index)>0)
			{
				...
				write.table(canceledDf, dockScheduleCancelFnUnload, sep=",", append=TRUE,col.names=FALSE,row.names=FALSE)
				write.csv(dockScheduleDfSub,dockScheduleFnUnload,row.names=F)
			}
			...
			output$mainResultUI=renderUI({
				list(
					...
					,box(width=12,title='Recommended jobs to replace canceled slots',solidHeader=T,status='primary'
						,renderDT({
							jobsDf=read.csv(dockJobsUnload,header=T,stringsAsFactors=F)
	...
	observeEvent(input$overviewBtn, {
		...
		observeEvent(input$overviewDateBtn, {
			...
			overviewTimeSheet = data.frame(rbindlist(lapply(1:nrow(warehouseDf), function(i){
				...
				timeSheet = computetimeSheet(dockScheduleFnUnload,warehouseID, overviewDate)
	...
	observeEvent(input$WHsubmitBtn, {
		...
		timeSheet = computetimeSheet(dockScheduleFnUnload,warehouseSel, bookDate)
		output$timeDT = renderDT({
	...
	observeEvent(input$bookingDetBtn, {
		...
		}else
		{
			...
			write.table(newBookingDf, dockScheduleFnUnload, sep=",", append=TRUE,col.names=FALSE,row.names=FALSE)
...
# Page Config
#################################
AABasePageConfig <- list(
  
  # Title for menu
  'title' = 'Warehouse Unload',
